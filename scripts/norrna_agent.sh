...
# Norrna Agent 一键部署脚本
# 用法：
#   安装（交互模式）: ./norrna_agent.sh
#   安装（非交互模式）: ./norrna_agent.sh server=1.1.1.1:13001 apikey=YOUR_KEY dns=223.5.5.5:53,119.29.29.29:53
#   更新: ./norrna_agent.sh update
#   卸载: ./norrna_agent.sh uninstall
...
show_help() {
    echo ""
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}   Norrna Agent 部署脚本${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    echo -e "${BLUE}用法：${NC}"
...
generate_config() {
    ...
    cat > "$INSTALL_DIR/norrna.conf" << CONFEOF
{
  "dns": {
    "mode": "ipv4_then_ipv6",
...
CONFEOF
    ...
    log_success "配置文件已生成: $INSTALL_DIR/norrna.conf"
}
...
ExecStart=$INSTALL_DIR/norrna api -c $INSTALL_DIR/norrna.conf --server "$SERVER_ADDR" --key "$API_KEY"
...
show_info() {
    echo ""
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}   Norrna Agent 安装完成！${NC}"
    echo -e "${GREEN}========================================${NC}"
    echo ""
    echo -e "${BLUE}安装信息：${NC}"
    echo -e "  安装目录: ${YELLOW}$INSTALL_DIR${NC}"
    echo -e "  配置文件: ${YELLOW}$INSTALL_DIR/norrna.conf${NC}"
...
# 更新功能
update() {
    log_info "开始更新Norrna Agent..."
    
    # 检查是否已安装
    if [ ! -f "$INSTALL_DIR/norrna" ]; then
        log_error "未找到Norrna程序，请先运行安装"
        log_info "安装命令: $0"
        exit 1
    fi
    ...
    # 备份当前版本
    log_info "备份当前版本..."
    local backup_file="$INSTALL_DIR/norrna.backup.$(date +%Y%m%d_%H%M%S)"
    cp "$INSTALL_DIR/norrna" "$backup_file"
    log_success "已备份到: $backup_file"
    
    # 获取当前版本信息（如果可用）
    local old_version="未知"
    if "$INSTALL_DIR/norrna" --version > /dev/null 2>&1; then
        old_version=$("$INSTALL_DIR/norrna" --version 2>&1 | head -n1)
    fi
    ...
    if [[ -n "$BINARY_URL" ]] && wget -q --show-progress -O "$tmp_file" "$BINARY_URL"; then
        ...
        # 替换二进制文件
        mv "$tmp_file" "$INSTALL_DIR/norrna"
        chmod +x "$INSTALL_DIR/norrna"
        
        # 获取新版本信息
        local new_version="未知"
        if "$INSTALL_DIR/norrna" --version > /dev/null 2>&1; then
            new_version=$("$INSTALL_DIR/norrna" --version 2>&1 | head -n1)
        fi
        ...
        log_error "下载失败，恢复备份..."
        mv "$backup_file" "$INSTALL_DIR/norrna"
        chmod +x "$INSTALL_DIR/norrna"
        log_info "已恢复到原版本"
        exit 1
    fi
